    ...
    if request.method != "POST":
        ...
        try:
            ...
            if profile_pic:
                ...
                if compressed_image:
                    image_file = ContentFile(compressed_image, name=f"profile_{staff.id}.jpg")
                    staff.profile_pic.save(f"profile_{staff.id}.jpg", image_file, save=True)
                    logger.info("✅ Staff profile picture saved and compressed.")
                else:
                    logger.warning("⚠️ Failed to compress profile picture.")
            else:
                logger.warning("⚠️ No profile picture provided.")
            staff.save()
            if profile_pic:
                process_face_encoding(staff.id, profile_pic.read())
                messages.success(request, "Staff added successfully! Face encoding is being processed.")
            else:
                messages.warning(request, "Staff added, but no profile picture provided.")
            return HttpResponseRedirect(reverse("add_staff"))
        except Exception as e:
...
def add_student_save(request):
    ...
    if form.is_valid():
        try:
            ...
            if profile_pic:
                process_face_encoding(student.id, profile_pic.read())
...
# Delete Operations for Staff, Subject, and Student ==================================================================================================================================
...
def clear_used_rfid_tags():
    try:
        ...
        with open("rfid_tags.txt", "w") as file:
            file.write("\n".join(remaining_tags))
        open("used_rfid_tags.txt", "w").close()
...
# Start RFID reader and auto-clear system on startup ===================================================================
